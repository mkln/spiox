...
spiox <- function(Y, X, coords, m = 15, 
  ...
  fit    <- match.arg(fit)
  vi_missing_smp = as.integer(vi_missing_smp)
  ...
  stopifnot(
    ...
    "vi_missing_smp must be a nonnegative integer" = length(vi_missing_smp) == 1L && vi_missing_smp >= 0L
  )
  ...
  # Latent VI prediction output ordering
  if ("Yhat" %in% names(out)) {
    out$Yhat <- out$Yhat[order(dag$order),,,drop=FALSE]
  }
